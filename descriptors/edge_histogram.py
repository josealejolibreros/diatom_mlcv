...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 3:
        print(
        'Please, specify destination descriptor filename.')
        sys.exit(0)
    ...
    subimage_height = int(image.shape[0] / 4)
    subimage_width = int(image.shape[1] / 4)

    block_size = int(np.floor(np.sqrt(subimage_height * subimage_width / 1100) / 2) * 2)
    block_width = block_size
    block_height = block_size
    blocks_per_subimage = int(subimage_height / block_height * subimage_width / block_width)
        ...
        for j in np.arange(0, image.shape[1], subimage_width):
            ...
            for ii in np.arange(0, subimage.shape[0], block_height):
                for jj in np.arange(0, subimage.shape[1], block_width):
                    ...
                    subblock1_mean = np.mean(block[0:int(block.shape[0] / 2), 0:int(block.shape[1] / 2)])
                    subblock2_mean = np.mean(block[0:int(block.shape[0] / 2), int(block.shape[1] / 2):block.shape[1]])
                    subblock3_mean = np.mean(block[int(block.shape[0] / 2):block.shape[0], 0:int(block.shape[1] / 2)])
                    subblock4_mean = np.mean(
                        block[int(block.shape[0] / 2):block.shape[0], int(block.shape[1] / 2):block.shape[1]])
    ...
    for i in range(16):
        np.add(quant_hist[(i * 5):(i * 5 + 5)], np.abs(np.ones([8, 1]) * hists[i, :] - QUANTIZER_MATRIX).argmin(axis=0),
               out=quant_hist[(i * 5):(i * 5 + 5)], casting="unsafe")
    ...
    #        np.ones([8, 1]) * semiglobal_hist[i, :] - QUANTIZER_MATRIX).argmin(axis=0)
    for i in range(13):
        np.add(quant_semiglobal_hist[(i * 5):(i * 5 + 5)],
               np.abs(np.ones([8, 1]) * semiglobal_hist[i, :] - QUANTIZER_MATRIX).argmin(axis=0),
